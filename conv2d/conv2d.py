...
from conv2d.inverses.inverse_cython import Inverse, Inverse_optim
...
def invertible_conv2D(
    ...
    with tf.variable_scope(name):
        ...
        def forward_fast(z, logdet):
            ...
            logdet += tf.reduce_sum(log_abs_diagonal(w)) * (height * width)
            if checkpoint_fn is not None:
                checkpoint_fn(z, logdet)
        ...
        if not reverse:
            ...
            x = tf.py_func(
                Inverse_optim(is_upper=1, dilation=dilation),
                inp=[z, w, b],
                Tout=tf.float32,
                stateful=True,
                name='conv2dinverse2',)
