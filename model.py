...
from conv2d.conv2d import invertible_conv2D_emerging, invertible_ar_conv2D, invertible_conv2D
...
def revnet2d_step(name, z, logdet, hps, reverse):
    with tf.variable_scope(name):
        ...
        if not reverse:
            ...
            elif hps.flow_permutation == 6:
                z, logdet = invertible_1x1_conv("invconv", z, logdet, decomposition=hps.decomposition)
                z, logdet = invertible_conv2D("conv", z, logdet, checkpoint_fn=checkpoint)
            elif hps.flow_permutation == 7:
                z, logdet = invertible_conv2D("conv", z, logdet, checkpoint_fn=checkpoint)
            else:
                raise Exception()
            
            if hps.flow_coupling <= 1:
                z1 = z[:, :, :, :n_z // 2]
                z2 = z[:, :, :, n_z // 2:]
            ...
            if hps.flow_coupling <= 1:
                ...
                h = f("f1", x1, hps.width, 2 * block_size)
                shift = h[:, :, :, 0::2]
                # scale = tf.exp(h[:, :, :, 1::2])
                scale = tf.nn.sigmoid(h[:, :, :, 1::2] + 2.)
                logscale = tf.log_sigmoid(h[:, :, :, 1::2] + 2.)
                x2 = z[:,:,:,block_size:2 * block_size] / scale
                x2 -= shift
                logdet -= tf.reduce_sum(logscale, axis=[1, 2, 3])

                h = f("f2", tf.concat([x1,x2],3), hps.width, 2 * block_size)
                shift = h[:, :, :, 0::2]
                # scale = tf.exp(h[:, :, :, 1::2])
                scale = tf.nn.sigmoid(h[:, :, :, 1::2] + 2.)
                logscale = tf.log_sigmoid(h[:, :, :, 1::2] + 2.)
                x3 = z[:,:,:,2*block_size:3 * block_size] / scale
                x3 -= shift
                logdet -= tf.reduce_sum(logscale, axis=[1, 2, 3])

                h = f("f3", tf.concat([x1,x2,x3],3), hps.width, 2 * block_size)
                shift = h[:, :, :, 0::2]
                # scale = tf.exp(h[:, :, :, 1::2])
                scale = tf.nn.sigmoid(h[:, :, :, 1::2] + 2.)
                logscale = tf.log_sigmoid(h[:, :, :, 1::2] + 2.)
                x4 = z[:,:,:,3*block_size:] / scale
                x4 -= shift
                logdet -= tf.reduce_sum(logscale, axis=[1, 2, 3])

                z = tf.concat([x1,x2,x3,x4],3)
            else:
                raise Exception()
            ...
            if hps.flow_permutation == 0:
                ...
                z, logdet = invertible_conv2D(
                    "conv", z, logdet, reverse=True)
                z, logdet = invertible_1x1_conv(
                    "invconv", z, logdet, reverse=True, decomposition=hps.decomposition)
            elif hps.flow_permutation == 7:
                z, logdet = invertible_conv2D(
